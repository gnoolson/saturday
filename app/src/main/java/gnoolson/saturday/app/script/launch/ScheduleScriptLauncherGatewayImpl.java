...
@Log4j2
@Component
public class ScheduleScriptLauncherGatewayImpl extends CommonScriptLauncher implements ScheduleScriptLauncherGateway {
     ...
     *
     * */
    public ScheduleScriptLauncherGatewayImpl(ScriptErrorHandler scriptErrorHandler,
    ...
    @Override
    public void execute(ScriptId scriptId, ScheduleId scheduleId) {
        try (Locker.LockHandle ignore = locker.lockIds(LockId.of(scriptId))) {
            ...
            argsData.put(DefaultField.SOURCE, "SCHEDULE");
            argsData.put("scheduleId", scheduleId.getValue().toString());
